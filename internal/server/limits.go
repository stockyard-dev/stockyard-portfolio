...
func DefaultLimits() Limits {
	...
	claims := validateLicenseKey(key, "portfolio")
	if claims == nil {
	...
	if claims.Tier != "individual" && claims.Tier != "*" {
		...
		for _, t := range claims.Tools {
			if t == "portfolio" || t == "*" { found = true; break }
		}
		if !found {
			log.Printf("[license] Tool portfolio not in licensed tools")
			return NoLicense()
		}
	}
...
func (s *Server) tierHandler(w http.ResponseWriter, r *http.Request) {
	...
	m["upgrade_url"] = "https://stockyard.dev/portfolio/"
	wj(w, 200, m)
}
