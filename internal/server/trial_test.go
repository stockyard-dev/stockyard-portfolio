...
// Tests for the additive trial-gate middleware on portfolio. Booking
...
//   7. End-to-end: tier=="none" → POST /api/projects returns 402
//   8. End-to-end: tier=="paid" → POST /api/projects returns 200
//   9. End-to-end: tier=="none" → GET /api/projects still works (read-through)
...
// portfolio's existing limits.go doesn't expose a hook for that.

func newTestDB(t *testing.T) *store.DB {
	t.Helper()
	dir, err := os.MkdirTemp("", "portfolio-trial-test-*")
	if err != nil {
...
func newTestServer(t *testing.T, tier string) *Server {
	t.Helper()
	dir, err := os.MkdirTemp("", "portfolio-trial-srv-*")
	if err != nil {
		t.Fatalf("mkdtemp: %v", err)
	}
	t.Cleanup(func() { os.RemoveAll(dir) })
...
func TestShouldBlockWrite_NoneTier(t *testing.T) {
	...
	for _, c := range cases {
		req := httptest.NewRequest(c.method, "/api/projects", nil)
...
func TestShouldBlockWrite_PaidTier(t *testing.T) {
	...
	for _, m := range []string{"POST", "PUT", "DELETE", "PATCH", "GET"} {
		req := httptest.NewRequest(m, "/api/projects", nil)
		if s.shouldBlockWrite(req) {
...
func TestEndToEnd_NoneTierBlocksWrite(t *testing.T) {
	s := newTestServer(t, "none")
	body := bytes.NewBufferString(`{"title":"Q4 Project"}`)
...
func TestPersistLicense_WritesFile(t *testing.T) {
	dir, err := os.MkdirTemp("", "portfolio-persist-test-*")
	if err != nil {
		t.Fatalf("mkdtemp: %v", err)
	}
...
func TestLoadLicenseFromDisk_RoundTrip(t *testing.T) {
	dir, err := os.MkdirTemp("", "portfolio-load-test-*")
	if err != nil {
		t.Fatalf("mkdtemp: %v", err)
	}
	defer os.RemoveAll(dir)
...
func TestLoadLicenseFromDisk_MissingReturnsEmpty(t *testing.T) {
	dir, err := os.MkdirTemp("", "portfolio-missing-test-*")
	if err != nil {
		t.Fatalf("mkdtemp: %v", err)
	}
	defer os.RemoveAll(dir)
