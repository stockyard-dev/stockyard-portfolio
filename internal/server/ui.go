...
const dashHTML = `<!DOCTYPE html><html><head><meta charset="UTF-8"><meta name="viewport" content="width=device-width,initial-scale=1.0"><title>Portfolio</title>
...
<div class="hdr"><h1><span>&#9670;</span> PORTFOLIO</h1><button class="btn btn-p" id="add-btn">+ Add</button></div>
<div class="main">
<div class="stats" id="stats-projects"></div>
<div class="toolbar">
<input class="search" id="search-projects" placeholder="Search..." oninput="renderRes('projects')"><select class="filter-sel" id="filter-projects-status" onchange="renderRes('projects')"><option value="">All Status</option><option value="Draft">Draft</option><option value="Published">Published</option><option value="Archived">Archived</option></select></div>
<div class="count-label" id="count-projects"></div>
<div class="tbl-wrap"><table class="tbl"><thead><tr><th>Title</th><th>Category</th><th>Client</th><th>Date</th><th>Description</th><th>Image URL</th><th>Project URL</th><th>Tags</th><th>Featured</th><th>Status</th><th></th></tr></thead><tbody id="tbody-projects"></tbody></table></div></div><div class="modal-bg" id="mbg" onclick="if(event.target===this)closeModal()"><div class="modal" id="mdl"></div></div><script>var A="/api",data={},editId=null,activeRes="projects";var resCfg={"projects":{d:"Projects",f:[{n:"title",l:"Title",t:"text",r:true,o:[]},{n:"category",l:"Category",t:"text",r:false,o:[]},{n:"client",l:"Client",t:"text",r:false,o:[]},{n:"date",l:"Date",t:"date",r:false,o:[]},{n:"description",l:"Description",t:"textarea",r:false,o:[]},{n:"image_url",l:"Image URL",t:"url",r:false,o:[]},{n:"project_url",l:"Project URL",t:"url",r:false,o:[]},{n:"tags",l:"Tags",t:"text",r:false,o:[]},{n:"featured",l:"Featured",t:"checkbox",r:false,o:[]},{n:"status",l:"Status",t:"select",r:false,o:["Draft","Published","Archived"]}]},};
...
if(typeof renderRes==='function'){renderRes('projects')}
}else{
